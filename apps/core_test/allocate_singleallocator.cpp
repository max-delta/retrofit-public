...
template<size_t MaxTotalSize>
using SingleAllocatorT = AllocatorT<SingleAllocator<MaxTotalSize>>;
...
TEST( SingleAllocator, RFTLContainers )
{
	using IntAllocator = RftlAllocator<int>;
	using KVAllocator = RftlAllocator<rftl::pair<int const, int>>;

	details::SingleAllocatorT<1024> stackAlloc{ ExplicitDefaultConstruct() };
