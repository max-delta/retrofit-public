...
bool CharacterValidator::LoadGeneticsTable( file::VFSPath const& geneticsTablePath )
{
	...
	while( geneticsTable.empty() == false )
	{
		...
		if( entry.size() != 5 )
		{
			RFLOGF_NOTIFY( geneticsTablePath, RFCAT_CHAR, "Malformed genetics entry at line {}, expected 5 columns", line );
			return false;
		}
		...
		if( id.empty() )
		{
			RFLOGF_NOTIFY( geneticsTablePath, RFCAT_CHAR, "Malformed genetics entry at line {}, not a valid id", line );
			return false;
		}

		if( species.empty() )
		{
			RFLOGF_NOTIFY( geneticsTablePath, RFCAT_CHAR, "Malformed genetics entry at line {}, not a valid species", line );
			return false;
		}

		if( gender.empty() )
		{
			RFLOGF_NOTIFY( geneticsTablePath, RFCAT_CHAR, "Malformed genetics entry at line {}, not a valid gender", line );
			return false;
		}
		...
		{
			RFLOGF_NOTIFY( geneticsTablePath, RFCAT_CHAR, "Malformed genetics entry at line {}, not a valid playability", line );
			return false;
		}

		if( nineChar.empty() )
		{
			RFLOGF_NOTIFY( geneticsTablePath, RFCAT_CHAR, "Malformed genetics entry at line {}, not a valid 9-char", line );
			return false;
		}
...
bool CharacterValidator::LoadStatBonusesTable( file::VFSPath const& statBonusesTablePath )
{
	...
	while( statBonusesTable.empty() == false )
	{
		...
		if( entry.size() != 9 )
		{
			RFLOGF_NOTIFY( statBonusesTablePath, RFCAT_CHAR, "Malformed stat bonuses entry at line {}, expected 9 columns", line );
			return false;
		}
		...
		if( species.empty() )
		{
			RFLOGF_NOTIFY( statBonusesTablePath, RFCAT_CHAR, "Malformed stat bonuses entry at line {}, not a valid species", line );
			return false;
		}
		...
		if( validStats == false )
		{
			RFLOGF_NOTIFY( statBonusesTablePath, RFCAT_CHAR, "Malformed stat bonuses entry at line {}, not a valid modifier", line );
			return false;
		}
