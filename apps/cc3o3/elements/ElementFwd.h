...
inline constexpr ElementIdentifier kInvalidElementIdentifier = 0;
...
inline constexpr InnateIdentifier kInvalidInnateIdentifier = 0;

using ElementLevel = uint8_t;
inline constexpr ElementLevel kInvalidElementLevel = 0;
inline constexpr ElementLevel kMinElementLevel = 1;
inline constexpr ElementLevel kMaxElementLevel = 8;
inline constexpr size_t kNumElementLevels = kMaxElementLevel - kMinElementLevel + 1;

// Convenience for ranged-based for-loops
// NOTE: Defeats optimization, be mindful in performance-critical code
inline constexpr ElementLevel kElementLevels[] = { 1, 2, 3, 4, 5, 6, 7, 8 };
static_assert( sizeof( kElementLevels ) == sizeof( ElementLevel ) * kNumElementLevels );

// Convenience for zero-based container logic
inline constexpr size_t AsLevelOffset( ElementLevel level )
{
	return angry_cast<ElementLevel>( level - kMinElementLevel );
}
inline constexpr ElementLevel FromLevelOffset( size_t offset )
{
