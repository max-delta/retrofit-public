...
bool PageMapper::InitializeFromCharmapFile( file::VFS const& vfs, file::VFSPath const& path )
{
	...
	for( size_t i_row = 0; i_row < numRows; i_row++ )
	{
		...
		{
			RFLOGF_NOTIFY( path, RFCAT_LOCALIZATION, "Line %llu has invalid 'to' column, expected a number between 1 and 255", i_row );
			return false;
		}
...
rftl::string PageMapper::MapTo8Bit( rftl::u32string const& codePoints ) const
{
	...
	for( char32_t const& codePoint : codePoints )
	{
		...
		{
			RFLOGF_NOTIFY( nullptr, RFCAT_LOCALIZATION, "Failed to map the character %lu to an 8bit character", static_cast<uint32_t>( codePoint ) );
