...
bool CharacterCompositor::LoadPieceTable( rftl::string mode, CharacterPieceType pieceType, file::VFSPath const& pieceTablePath )
{
	...
	while( pieceTable.empty() == false )
	{
		...
		if( entry.size() != 2 )
		{
			RFLOGF_NOTIFY( pieceTablePath, RFCAT_GAMESPRITE, "Malformed master piece entry at line %i, expected 2 columns", math::integer_cast<int>( line ) );
			return false;
		}
		...
		if( id.empty() )
		{
			RFLOGF_NOTIFY( pieceTablePath, RFCAT_GAMESPRITE, "Malformed piece entry at line %i, bad id", math::integer_cast<int>( line ) );
			return false;
		}
		...
		{
			RFLOGF_NOTIFY( pieceTablePath, RFCAT_GAMESPRITE, "Malformed piece entry at line %i, bad resources", math::integer_cast<int>( line ) );
			return false;
		}
