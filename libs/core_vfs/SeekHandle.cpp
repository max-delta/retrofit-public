...
SeekHandle::SeekHandle( UniquePtr<rftl::streambuf>&& streamBuf )
...
size_t SeekHandle::GetSeekableSize() const
{
	RF_ASSERT( mStreamBuf != nullptr );
...
size_t SeekHandle::ReadBytes( void* buffer, size_t numBytes )
{
...
size_t SeekHandle::WriteBytes( void const* buffer, size_t numBytes )
{
	RF_ASSERT( buffer != nullptr );
	RF_ASSERT( numBytes > 0 );
...
WeakPtr<rftl::streambuf> SeekHandle::GetUnsafeTransientStreamBuffer()
{
	RF_ASSERT( mStreamBuf != nullptr );
	return mStreamBuf;
}
