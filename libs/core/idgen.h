...
public:
	...
	static constexpr ID kFinal = rftl::numeric_limits<ID>::max();
private:
	using Storage = rftl::atomic<ID>;
	static constexpr bool kRolloverUnrealistic = kInitial > 0 || kFinal < rftl::numeric_limits<int64_t>::max();
...
public:
	...
	ID Generate()
	{
		...
		ID newID = mStorage.fetch_add( 1, rftl::memory_order::memory_order_acq_rel );
